...
function getFilteredTotalGoals(minGoals, teamPlayers) {
    let filteredPlayer = teamPlayers.filter(player => player.goals >= minGoals);
    let totalGoals = filteredPlayer.reduce((total,player) => total + player.goals,0);
    alert(`Tổng số bàn thắng của các cầu thủ đạt đủ chỉ tiêu là : ${totalGoals}`);
}

getFilteredTotalGoals(0, players);